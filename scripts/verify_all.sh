...
run "backend (8765)"  "curl -sf $BE/api/health"               '"status":"ok"'
...
async def go():
    ...
    try:
        async with httpx.AsyncClient(timeout=10) as cl:
            r = await cl.post('http://localhost:8765/api/calls/start', json={
                'bot_id': 1,
                'vars': {'customer_name': '홍길동'},
            })
            sid = r.json()['session_id']
        async with websockets.connect(f'ws://localhost:8765/ws/calls/{sid}') as ws:
            received = None
            for _ in range(15):
                try:

...
async def list_skills(bot_id: int, svc: SkillService = Depends(get_skill_service)):
    return [_to_out(s) for s in await svc.list_by_bot(bot_id)]


@router.get("/{skill_id}", response_model=schemas.SkillOut)
async def get_skill(skill_id: int, svc: SkillService = Depends(get_skill_service)):
    s = await svc.get(skill_id)
    if not s:
...
async def create_skill(payload: schemas.SkillCreate, svc: SkillService = Depends(get_skill_service), db: AsyncSession = Depends(get_db)):
...
async def update_skill(skill_id: int, payload: schemas.SkillUpdate, svc: SkillService = Depends(get_skill_service)):
    try:
        s = await svc.update(skill_id, **payload.model_dump(exclude_unset=True))
    except DomainError as e:
        msg = str(e)
...
async def delete_skill(skill_id: int, svc: SkillService = Depends(get_skill_service)):

...
def list_tenants(svc: TenantService = Depends(get_tenant_service)):
    return [_to_out(t) for t in svc.list()]


@router.get("/{tenant_id}", response_model=schemas.TenantOut)
def get_tenant(tenant_id: int, svc: TenantService = Depends(get_tenant_service)):
    t = svc.get(tenant_id)
    if not t:
...
def create_tenant(payload: schemas.TenantCreate, svc: TenantService = Depends(get_tenant_service)):
    try:
        t = svc.create(name=payload.name, slug=payload.slug)
...
def delete_tenant(tenant_id: int, svc: TenantService = Depends(get_tenant_service)):
